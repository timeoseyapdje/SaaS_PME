...
export async function POST(request: Request) {
  ...
  try {
    ...
    try {
      ...
      console.error("Erreur création notifications DB:", dbErr);
      return NextResponse.json(
        { error: "Erreur lors de la sauvegarde des notifications" },
        { status: 500 }
      );
    }
    ...
    if (apiKey) {
      try {
        ...
        console.error("Erreur import Resend:", importErr);
        // Continue without email - notifications are already saved in DB
      }
    }
    ...
    return NextResponse.json({
      ...
      emailsSent,
      emailsFailed,
      message: `Notification envoyée à ${users.length} utilisateur(s)${emailsSent > 0 ? ` (${emailsSent} email(s))` : ""}`,
    });
  } catch (err) {
